...
export const nextStepIsAvailable = (rover: Rover, inputLocation: World): boolean => {
  switch (rover.position.pointer) {
    case 'N':
      if (rover.position.coordinates.latitude >= inputLocation.height) {
        return false;
      }
      break;
    case 'S':
      if (rover.position.coordinates.latitude <= 0) {
        return false;
      }
      break;
    case 'E':
      if (rover.position.coordinates.longitude >= inputLocation.width) {
        return false;
      }
      break;
    case 'W':
      if (rover.position.coordinates.longitude <= 0) {
        return false;
      }
      break;
  }
